...
public class PlayerManager : MonoBehaviour, IPlayerManager
{
    ...
    )
    {
        ...
        if (this.collParent) bodyObj.transform.SetParent(this.collParent.transform);
    ...
    // defaulted to 60 fps
    void updateCoordinate(float delta = 0.016f)
    {
                ...
                if (bodyState != null)
                {
                    if (ii != 0)
                    {
                        ...
                        string coordName = AStarFunctions.GetStringCoordName(new Vector2(
                            bodyState.Obj?.transform.position.x ?? headX,
                            bodyState.Obj?.transform.position.y ?? headY
                        ));
                    ...
                    finalPos = bodyState.Obj?.transform.position ?? new Vector3(0, 0);
                    bodyState.Position.Set(finalPos.x, finalPos.y);
